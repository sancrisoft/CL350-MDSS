    ...
    if (!this.__is_initialized) {
        ...
        if (this.__api) {
            ...
            if (this.__api_version == "1.2") {
                ...
                var lessonStatus = this.__api.LMSGetValue("cmi.core.lesson_status");
                if(lessonStatus != "completed" && lessonStatus != "passed") {
                    this.__api.LMSSetValue("cmi.core.lesson_status", "incomplete");
                }
            }
            // SCORM 2004
            if (this.__api_version == "2004") {
                ...
                var lessonStatus = this.__api.LMSGetValue("cmi.core.lesson_status");
                if(lessonStatus != "completed" && lessonStatus != "passed") {
                    this.__api.setValue("cmi.core.lesson_status", "incomplete");
                }
            }
        }
    }
...
function batcGateKeeper( debug ) {
	this.WBT_LOGGER_API = "../../../wbtlogger/wbtlProxy.php";
    this._debug = debug;
    this._debug_string = "";
    // wbtl data
    this._studentID 	= "";
